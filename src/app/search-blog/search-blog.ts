...
export class SearchBlog {
  ...
  postLength(categoryName: string): number {
    this.selectedCategory = categoryName;

    if (categoryName === 'all') {
      return this.posts.posts.length;
    }

    const filteredPosts = this.posts.posts.filter((post) => post.category === categoryName);

    return filteredPosts.length;
  }

  getCategoryName(): string {
    return this.selectedCategory === 'all' ? 'جميع الفئات' : this.selectedCategory;
  }
  getFilter = (): post[] => {
    let filteredPosts: post[] = [];

    if (this.selectedCategory === 'all') {
      filteredPosts = this.posts.posts;
    } else {
      filteredPosts = this.posts.posts.filter((post) => post.category === this.selectedCategory);
    }

    return filteredPosts;
  };
  // function of dates
  getArabicMonth(data: string): string {
    const dateObj = new Date(data);
    const monthNumber = dateObj.getMonth() + 1;
    const year = dateObj.getFullYear();
    const days = dateObj.getDate();
    return `${days}-${this.monthNames[monthNumber]}-${year}`;
  }
  isGridActive = true;
  isListActive = false;

  handleClick(typeView: string) {
    if (typeView === 'Grid') {
